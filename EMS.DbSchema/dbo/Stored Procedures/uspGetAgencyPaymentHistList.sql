...
CREATE PROCEDURE [dbo].[uspGetAgencyPaymentHistList]
(
	@iAgencyHistID INT = NULL,
	@iAgencyID INT
)
AS
BEGIN
	BEGIN TRY
		...
		SELECT
			...
			szUpdatedBy
		FROM
			tblAgencyPaymentHist (NOLOCK)
		WHERE
			((@iAgencyHistID IS NULL) OR (iAgencyHistID = @iAgencyHistID)) AND
