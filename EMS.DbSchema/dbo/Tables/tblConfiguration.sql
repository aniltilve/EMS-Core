CREATE TABLE [dbo].[tblConfiguration] (
    [szAdminPassword] VARCHAR (128) NOT NULL,
    [iPort]           INT           NOT NULL,
    [szMailFrom]      VARCHAR (128) NOT NULL,
    [szMailPassword]  VARCHAR (128) NOT NULL,
    [szMailSubject]   VARCHAR (128) NOT NULL,
    [szSMTPServer]    VARCHAR (64)  NOT NULL,
    [bIsMailRequired] BIT           NOT NULL,
    [szVersion]       VARCHAR (16)  NOT NULL,
    [dtUpdated]       DATETIME      NOT NULL,
    [szUpdatedBy]     VARCHAR (64)  NOT NULL,
    [szKey]           VARCHAR (8)   NOT NULL,
    CONSTRAINT [PK_tblConfiguration] PRIMARY KEY CLUSTERED ([szKey] ASC)
);


GO

CREATE TRIGGER tblConfiguration_ChangeTracking ON tblConfiguration FOR INSERT, UPDATE
AS
...
SET NOCOUNT ON 
SELECT @TableName = 'tblConfiguration'
